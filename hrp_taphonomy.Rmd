...
missing <- setdiff(hadar_df$id, hadar_merge$id) #find missing data
...
#Barplot
biv_plot <- ggplot(count_data, aes(x=expected_count, fill = regions)) + geom_bar(stat= "identity", position = "dodge") #create barplot
biv_plot
```
...
```{r}
#UPDATE 
hd_chisq <- na.omit(hadar_merge[,c("anatomical_region.y", "completeness")])
